...
// UpdateSupportJoin 解决gorm的update操作不支持join的问题
// 使用方法:
//  1. 直接使用: db.Table(xx).Join(xx).Where(xx).Model(&UpdateSupportJoin{}).Updates(xx)
//  2. 嵌套使用: type yy struct{UpdateSupportJoin}; db.Table(xx).Join(xx).Where(xx).Model(&yy{}).Updates(xx)
type UpdateSupportJoin struct{}

func (u *UpdateSupportJoin) BeforeUpdate(tx *gorm.DB) (err error) {
	if tx == nil || tx.Statement == nil || len(tx.Statement.Joins) == 0 {
		return nil
	}
	...
	tx.Statement.AddClause(updateClauseSupportJoin{Joins: joins})
	return nil
}
...
func (updateEx updateClauseSupportJoin) Name() string { return "UPDATE" }
