...
func NewNode(node uint32, forceBlockWhenAllConsumed bool) *Node {
	return &Node{
		mu:                        sync.Mutex{},
		epoch:                     time.UnixMilli(_epoch),
...
func (n *Node) Generate() *ID {
	...
	if n.forceBlockWhenAllConsumed {
		// fix(1): 检查边界由 _stepMax(4096) 改为 _stepMask(4095)。
		// 原逻辑在 n.step==4095 时不触发阻塞，直接令 n.step 变为 4096，
		// 4096 超出 12 位掩码范围，步进位溢出覆盖 node 位，
		// 与其他 ID 产生碰撞；应在步进到达上限前阻塞等待下一毫秒。
		// fix(2): 睡眠后刷新 cur。原代码睡眠后 cur 仍为旧值，
		// `cur > n.time` 判断不成立导致步进无法被重置，仍会溢出。
		if (cur < n.time) || (cur == n.time && n.step >= _stepMask) {
			// 额度消费完之后，强制阻塞，等待下一轮时间周期
			time.Sleep(time.Duration(11-(cur%10)) * time.Millisecond)
			// 睡眠后刷新 cur，确保能正确进入下一时间周期并重置步进
			cur = time.Since(n.epoch).Milliseconds()
		}
	}
